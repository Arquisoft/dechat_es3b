...
class QueryPath {
  constructor(settings = {}, data = {}) {
    // Set data fields
    Object.assign(this, data); // Create proxy object
  ...
  get(handlers, property) {
    ...
    if (handler && typeof handler.execute === 'function') return handler.execute(this); // Resolvers provide functionality for arbitrary properties,
    // so find a resolver that can handle this property

    for (const resolver of this._resolvers) {
      if (resolver.supports(property)) return resolver.resolve(this, property);
  ...
  extend(data) {
    return new QueryPath(this.settings, Object.assign({
      parent: this
    }, data));
  }
