...
const PRIVATE_CHAT_FOLDER = '/private/dechat';
...
export class ChatService {
  ...
  async init() {
    ...
    await this.createDeChatFolder();
    await this.loadTwoPersonChats();
  ...
  /**
   * Crea la carpeta private si no existe
   */
  async createPrivateFolder() {
    let folder = await this.readFolder(this.uri + PRIVATE_FOLDER);
    if (folder === undefined) {
  ...
  /**
   * Crea la carpeta para almacenar chats si no existe
   */
  async createDeChatFolder() {
 let checkFolder = await this.readFolder(this.uri + PRIVATE_CHAT_FOLDER);
  ...
  async loadTwoPersonChats() {
    ...
    for (const file of folderContent.files) {
      ...
      let chat:TwoPersonChat = JSON.parse(channelJsonld);
      this.chats.push(chat);
    }
  }

  /**
   * Devuelve al web base del pod del usuario
   */
  ...
  /**
   * Guarda el mensaje el mensaje en las usuarios participantes
   */
  async sendMessage(chatChannel: TwoPersonChat, msg: Message) {
    try {
      // Detenemos la comprobación del inbox temporalmente
      this.stopExternally();

      // Comprobamos que el canal exista
      let channel:TwoPersonChat = this.searchChatChannelById(chatChannel.id);
      if (channel != null) {
        ...
        chatChannel.person.forEach(async participant => {  // <<En este momento solo está implementado para cada persona distinta un chat distinto>>
  ...
  private async processNewMessage(urlFile: any) {
    ...
    let chat:TwoPersonChat = this.searchChatChannelByParticipantWebid(newMessage.makerWebId);
    if (chat != null) {
      chat.messages.push(newMessage);
      chat.messages.sort(function(a, b) { return  +new Date(a.sendTime) - +new Date(b.sendTime) });

      // Actualizamos chat en POD propio
      await this.updateFile(this.uri + PRIVATE_CHAT_FOLDER + "/" + chat.id + "." + MESSAGE_FILE_FORMAT, JSON.stringify(chat));
    } else {
      // Si no hay canal asociado creamos uno
      chat = new TwoPersonChat(this.getChatID(), "");
      chat.messages.push(newMessage);
      chat.person.push(newMessage.makerWebId); 
      this.chats.push(chat);

      // Añadimos chat a POD propio
      await this.createFile(this.uri + PRIVATE_CHAT_FOLDER + "/" + chat.id, JSON.stringify(chat), CHAT_CHANNEL_CONTENT_TYPE);
    }
  }
   ...
   */
  public async showChatMessages(chat: TwoPersonChat) {
   ...
   * @param title
   */
  public async createChat(webId: string, title?: string, message?: Message): Promise<TwoPersonChat> {
    let channel:TwoPersonChat = null;

    if (channel == null) {
      title = (title == undefined)? "Prueba_chat_inbox" : title;
      let newChatChannel = new TwoPersonChat(this.getChatID(), title);
      if (message != undefined) { newChatChannel.messages.push(message); }

      newChatChannel.person.push(webId);
      this.chats.push(newChatChannel);
   ...
   * @param webId
   */
  public searchChatChannelByParticipantWebid(webId: string): TwoPersonChat {
    for (const chat of this.chats) {
      if (chat.person.includes(webId)) {
        return chat;
      }
    }
    return null;
   ...
   * @param id
   */
  public searchChatChannelById(id: string): TwoPersonChat {
